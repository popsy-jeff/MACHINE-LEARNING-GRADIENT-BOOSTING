"""
feature_pipeline.py

Rebuilds the EXACT feature set the model was trained on, starting from
raw, human-entered inputs (store details + a date).

Per the workflow document (Phase 2): the feature engineering here must
produce identical columns, in identical order, to what the model saw
during training -- otherwise predictions silently go wrong.

IMPORTANT: This is a best-effort reconstruction based on the columns
you showed during development (Store, Promo, Promo2, SchoolHoliday,
CompetitionDistance, Year, Month, Day, WeekOfYear, CompetitionOpen,
Promo2Open, IsPromo2Month, plus one-hot StateHoliday/StoreType/
Assortment/DayOfWeek). Swap in your real feature_cols.pkl to guarantee
an exact match -- see load_feature_cols() below.
"""

import numpy as np
import pandas as pd
from datetime import date


# ------------------------------------------------------------------
# Column order — loaded from disk if available, else falls back to
# the column list observed during development. Always prefer the
# saved version, since it's guaranteed to match your trained model.
# ------------------------------------------------------------------
DEFAULT_FEATURE_COLS = [
    'Store', 'Promo', 'Promo2', 'SchoolHoliday', 'CompetitionDistance',
    'Year', 'Month', 'Day', 'WeekOfYear', 'CompetitionOpen',
    'Promo2Open', 'IsPromo2Month',
    'StateHoliday_0', 'StateHoliday_a', 'StateHoliday_b', 'StateHoliday_c',
    'StoreType_a', 'StoreType_b', 'StoreType_c', 'StoreType_d',
    'Assortment_a', 'Assortment_b', 'Assortment_c',
    'DayOfWeek_1', 'DayOfWeek_2', 'DayOfWeek_3', 'DayOfWeek_4',
    'DayOfWeek_5', 'DayOfWeek_6', 'DayOfWeek_7',
]


def load_feature_cols(path="models/feature_cols.pkl"):
    """Load the exact training-time column order if saved; else fall back."""
    import joblib
    import os
    if os.path.exists(path):
        return joblib.load(path)
    return DEFAULT_FEATURE_COLS


def _one_hot(value, categories, prefix):
    """Build a dict of one-hot columns for a single categorical value."""
    return {f"{prefix}_{c}": int(value == c) for c in categories}


def build_features(
    store_id: int,
    forecast_date: date,
    promo: int,
    promo2: int,
    school_holiday: int,
    state_holiday: str,          # one of '0', 'a', 'b', 'c'
    store_type: str,             # one of 'a', 'b', 'c', 'd'
    assortment: str,             # one of 'a', 'b', 'c'
    competition_distance: float,
    competition_open_since_year: int = None,
    competition_open_since_month: int = None,
    promo2_since_year: int = None,
    promo2_since_week: int = None,
    promo_interval_months: list = None,   # e.g. [1,4,7,10] if store runs Promo2 that month
    feature_cols: list = None,
) -> pd.DataFrame:
    """
    Turn raw, human-entered store + date info into a single-row dataframe
    matching the model's expected feature columns.

    This mirrors the derived-feature logic referenced in the workflow
    document (CompetitionOpen, Promo2Open, IsPromo2Month) plus the
    one-hot encodings used during training.
    """
    if feature_cols is None:
        feature_cols = DEFAULT_FEATURE_COLS

    year = forecast_date.year
    month = forecast_date.month
    day = forecast_date.day
    week_of_year = forecast_date.isocalendar()[1]
    day_of_week = forecast_date.isoweekday()  # 1=Monday ... 7=Sunday

    # --- CompetitionOpen: how many months the competitor has been open ---
    if competition_open_since_year and competition_open_since_month:
        competition_open = max(
            0,
            (year - competition_open_since_year) * 12
            + (month - competition_open_since_month),
        )
    else:
        competition_open = 0

    # --- Promo2Open: how many weeks this store's Promo2 has been running ---
    if promo2 and promo2_since_year and promo2_since_week:
        promo2_open = max(
            0,
            (year - promo2_since_year) * 52 + (week_of_year - promo2_since_week),
        )
    else:
        promo2_open = 0

    # --- IsPromo2Month: is the current month one of this store's Promo2 months ---
    is_promo2_month = int(promo2 == 1 and promo_interval_months and month in promo_interval_months)

    row = {
        'Store': store_id,
        'Promo': promo,
        'Promo2': promo2,
        'SchoolHoliday': school_holiday,
        'CompetitionDistance': competition_distance,
        'Year': year,
        'Month': month,
        'Day': day,
        'WeekOfYear': week_of_year,
        'CompetitionOpen': competition_open,
        'Promo2Open': promo2_open,
        'IsPromo2Month': is_promo2_month,
    }

    row.update(_one_hot(state_holiday, ['0', 'a', 'b', 'c'], 'StateHoliday'))
    row.update(_one_hot(store_type, ['a', 'b', 'c', 'd'], 'StoreType'))
    row.update(_one_hot(assortment, ['a', 'b', 'c'], 'Assortment'))
    row.update(_one_hot(day_of_week, [1, 2, 3, 4, 5, 6, 7], 'DayOfWeek'))

    df = pd.DataFrame([row])

    # Guarantee exact column order / presence expected by the model —
    # fill any missing engineered column with 0 rather than erroring out
    for col in feature_cols:
        if col not in df.columns:
            df[col] = 0
    df = df[feature_cols]

    return df


def build_features_batch(raw_df: pd.DataFrame, feature_cols: list = None) -> pd.DataFrame:
    """
    Same transformation as build_features(), applied to a batch CSV upload.
    Expects raw_df to have the same raw columns a user would fill in on
    the single-prediction form (see REQUIRED_BATCH_COLUMNS below).
    """
    if feature_cols is None:
        feature_cols = DEFAULT_FEATURE_COLS

    rows = []
    for _, r in raw_df.iterrows():
        f_date = pd.to_datetime(r['Date']).date()
        row_df = build_features(
            store_id=int(r['Store']),
            forecast_date=f_date,
            promo=int(r.get('Promo', 0)),
            promo2=int(r.get('Promo2', 0)),
            school_holiday=int(r.get('SchoolHoliday', 0)),
            state_holiday=str(r.get('StateHoliday', '0')),
            store_type=str(r.get('StoreType', 'a')),
            assortment=str(r.get('Assortment', 'a')),
            competition_distance=float(r.get('CompetitionDistance', 0)),
            competition_open_since_year=r.get('CompetitionOpenSinceYear'),
            competition_open_since_month=r.get('CompetitionOpenSinceMonth'),
            promo2_since_year=r.get('Promo2SinceYear'),
            promo2_since_week=r.get('Promo2SinceWeek'),
            promo_interval_months=None,
            feature_cols=feature_cols,
        )
        rows.append(row_df)
    return pd.concat(rows, ignore_index=True)


REQUIRED_BATCH_COLUMNS = [
    'Store', 'Date', 'Promo', 'Promo2', 'SchoolHoliday', 'StateHoliday',
    'StoreType', 'Assortment', 'CompetitionDistance',
]
