...
from utils.style import inject_css, hero, section_tag, badge, nav_card, COLORS
...
with status_col:
    if is_demo:
        badge("● Demo model", COLORS["medium"])
    else:
        badge("● Live model", COLORS["low"])
with version_col:
...
st.write("")
section_tag("Get around the app")
st.markdown("##### Jump to any stage of the pipeline")

row1 = st.columns(3)
row2 = st.columns(2)

pages = [
    ("🔮", "Single Prediction", "Forecast one store's sales for one day.", "pages/1_Single_Prediction.py"),
    ("📋", "Batch Prediction", "Upload a CSV of store/date rows, get forecasts for all of them.", "pages/2_Batch_Prediction.py"),
    ("📊", "Model Performance", "Validation metrics and feature importance.", "pages/3_Model_Performance.py"),
    ("💰", "Advance Calculator", "Turn a 90-day forecast into a financing offer.", "pages/4_Advance_Calculator.py"),
    ("🛡️", "Risk Dashboard", "Review flagged / high-risk applications and the audit log.", "pages/5_Risk_Dashboard.py"),
]
...
for (icon, title, desc, target), slot in zip(pages, slots):
    with slot:
        ...
        st.page_link(target, label=f"Open {title} →", icon=None)

st.write("")
st.divider()
st.caption(
    "Built by Jeff Mulele · Gradient Boosting portfolio project · "
    "[View source on GitHub](https://github.com/popsy-jeff/MACHINE-LEARNING-GRADIENT-BOOSTING)"
)
